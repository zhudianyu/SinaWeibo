...
- (void)encodeWithCoder:(NSCoder *)encoder{
	...
	while ((attributeName = [keyEnum nextObject])) {
		SEL getSel = NSSelectorFromString(attributeName);
		if ([self respondsToSelector:getSel]) {
			NSMethodSignature *signature = nil;//方法签名
			signature = [self methodSignatureForSelector:getSel];
            //从方法签名返回调用实例
			NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:signature];
			[invocation setTarget:self];//设置目标
			[invocation setSelector:getSel];//设置调用方法
			NSObject *valueObj = nil;
			[invocation invoke];//调用方法
			[invocation getReturnValue:&valueObj];//得到返回值
			if (valueObj) {
...
- (NSString *)cleanString:(NSString *)str {
    ...
    [cleanString replaceOccurrencesOfString:@"\n" withString:@""
                                    options:NSCaseInsensitiveSearch //大小写不区分
